...
Monopoly.setNextPlayerTurn = function(){
    ...
    if (Monopoly.doubleCounter==1){ //Natalya (added condition for double counter)
      var nextPlayerId = playerId;//Natalya (added condition for double counter)
    } else { //Natalya (added condition for double counter)
      var nextPlayerId = playerId + 1;
    }//Natalya (added condition for double counter)

    if ($(".player#player"+nextPlayerId).is(".player-broke")){ //Natalya (adding for broke option) to jump over the broke player
      while($(".player#player"+nextPlayerId).is(".player-broke")){
        nextPlayerId++; //Natalya (adding for broke option) to jump over the broke player
        if (nextPlayerId > $(".player").length){
            nextPlayerId = 1;
        }
      }
    }//Natalya (adding for broke option) to jump over the broke player
...
Monopoly.handleBuy = function(player,propertyCell,propertyCost){
    ...
    if (playersMoney < propertyCost){
        ...
        Monopoly.playSound("notallowed");  //Natalya (add sound lack money)
    }else{
...
Monopoly.handlePassedGo = function(){
    var player = Monopoly.getCurrentPlayer();
    Monopoly.updatePlayersMoney(player,(-1)*Monopoly.moneyAtStart/10); //Natalya (cahnged the sign to add money instead of reducing)
};
